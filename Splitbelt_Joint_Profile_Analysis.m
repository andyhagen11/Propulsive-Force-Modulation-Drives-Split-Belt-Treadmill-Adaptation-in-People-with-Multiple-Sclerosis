...
% Define the list of specific participants you want to be incdluded in analysis (first layer of fields in the structure)
 ...
 % Create variable names to assign to average output tables
    ...
    for q = 1:length(JointNames)
        ...
        for r = 1:length(phases)
            ...
            % Only use the speicific windows (e.g. (:, 20:100)) for ankle - it wont work for hip or knee
%             eval([phases{r}, '_', JointNames{q}, '_ROM = range(AllData);']);
