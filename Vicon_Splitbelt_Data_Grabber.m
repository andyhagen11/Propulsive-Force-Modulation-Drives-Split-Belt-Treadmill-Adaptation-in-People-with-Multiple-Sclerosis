%% Vicon Nexus Split-belt Data Grabber %%
% Originating Author: Andrew Hagen
% Last Revised: 10/13/2023

% This script uses data from Vicon Nexus during a split-belt treadmill adaptation paradigm to create a structure of kinematic and kinetic data organzied with each row in the final matrix being represetative of one gait cycle

% Creates multi-layer nested data structure that is additive for all participants with a total of 336 matricies for each participant
    % Additionally pumps out another data structure with each gait cycle interpolated to be 100 data points long

    %Layer 1 = Participant
    %Layer 2 = Phase (Baseline, Adaptation1 (10min), Catch, Adaptation2 (5min) - will have to run seperately for each phase since they are separate Vicon trials
...
%% Set up connection with Vicon Nexus and set path
    ...
    Alimb = inputdlg('More Affected Limb? (L or R): '); 
...
%% Make a strucutre of stance force in each plane for the less affected leg
% Use 10 * i because force is collected at 1000 Hz rather than 100 Hz for angles and events - the correct force index will be the angle index * 10
