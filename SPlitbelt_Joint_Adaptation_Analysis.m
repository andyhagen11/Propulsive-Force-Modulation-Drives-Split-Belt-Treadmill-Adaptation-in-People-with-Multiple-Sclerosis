...
% This script uses data from a structure to calculate ROM and Peak adaptation time series (by gait cycle) for all joints and forces in all phases of a split-belt treadmill adaptation paradigm
%   see Vicon_Splitbelt_Data_Grabber in repository for structure organization and creation using Vicon Nexus outputs
...
% Define the list of specific participants
...
Planes = fieldnames(GaitCycle_Data_Interpolated.SBMS_001.Baseline.More_Affected.Hip_Angles.Stance);
    ...
    for j = 1:(numel(Periods) - 1) % Excludes Adaptation 2
        for k = 1:numel(Limbs) % 2 Limbs
            for l = 7 % 6 Angles, excludes Force data
...
%% Horizontally concatenate stance and swing phases of all vairables
...
ExportPlot = inputdlg('Create plots? Y/N: ');
 if strcmp(ExportPlot,'Y')
    ...
    for i = 1:numel(Indices)
        ...
        % Plot the second segment starting at Adapt and cropp 300:500
